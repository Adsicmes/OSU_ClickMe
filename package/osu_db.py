...
class OsuDB:
    """
    操作由osu!.db生成的cache.db 返回指定结果
    """
    def __init__(self, database:str):
        cache = sqlite3.connect(database)
        self.cur = cache.cursor()

    def allBeatmapset(self) -> list:
        self.cur.execute("select beatmap_set_id from maps")
        return list(set(list(map(lambda x: x[0], self.cur.fetchall()))))
    
    def hash2sid(self, hash:str):
        self.cur.execute(f"select beatmap_set_id from maps where md5_hash = '{hash}'")
...
def create_db(filename, outfile):
    sql = sqlite3.connect(outfile)
    c = sql.cursor()
...
if __name__ == "__main__":
    ...
    a = OsuDB()
    s = a.hash2sid('4a3e56374b3acdfae26b72fc3f9dcf0')
    print(s)
